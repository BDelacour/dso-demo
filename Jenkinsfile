pipeline {
  environment {
    ARGO_SERVER = '34.133.36.67:32100'
    DEV_URL = 'http://34.133.36.67:30080/'
  }
  agent {
    kubernetes {
    ...
    stage('Package') {
      parallel {
        ...
        stage('Docker BnP') {
          steps {
            container('kaniko') {
              sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --insecure --skip-tls-verify --cache=true --destination=docker.io/delaco/dsodemo'
            }
          }
        }
      }
    ...
    stage('Image Analysis') {
      parallel {
        ...
        stage('Image Scan') {
          steps {
            container('docker-tools') {
             sh 'trivy image --exit-code 1 delaco/dsodemo'
            }
          }
        }
      }
